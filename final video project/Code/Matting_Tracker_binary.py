...
def Matting():
    ...
    for iteration in tqdm(range(0, min(n_frames_stab, n_frames_bin) - config.MAT_frame_reduction_DEBUG),position=0, leave=True):
        ...
        frame_stab_tracked = frame_stab[int(config.bbox[1]):int((config.bbox[1] + config.bbox[3])),
                             int(config.bbox[0]):int(1.3 * (config.bbox[0] + config.bbox[2])), :]
        gray_back_scrib = gray_back_scrib[int(config.bbox[1]):int((config.bbox[1] + config.bbox[3])),
                          int(config.bbox[0]):int(1.3 * (config.bbox[0] + config.bbox[2]))]
        gray_fore_scrib = gray_fore_scrib[int(config.bbox[1]):int((config.bbox[1] + config.bbox[3])),
                          int(config.bbox[0]):int(1.3 * (config.bbox[0] + config.bbox[2]))]

        frame_stab_hsv_tracked = cv2.cvtColor(frame_stab_tracked, cv2.COLOR_BGR2HSV)
        frame_stab_tracked = frame_stab_hsv_tracked[:, :, 2]

        # taking only points from scribble
        background_array = frame_stab_tracked[gray_back_scrib == 100]
        foreground_array = frame_stab_tracked[gray_fore_scrib == 100]

        # binary mask normalized to [0,1]
        frame_bin = frame_bin / 255

        # calculate KDE for background and foreground
        kde_fore_pdf = Matting_functions.kde_evaluate(foreground_array, forePlotted,
                                                      title='Kernel Density Estimation - Background')
        kde_back_pdf = Matting_functions.kde_evaluate(background_array, backPlotted,
                                                      title='Kernel Density Estimation - Foreground')
        ...
        foreground[int(config.bbox[1]): int(config.bbox[1] + config.bbox[3]),
        int(config.bbox[0]): int(1.3 * (config.bbox[0] + config.bbox[2])),
        0] = foreground_mask_tracked * frame_stab_hsv[int(config.bbox[1]): int(config.bbox[1] + config.bbox[3]),
                                       int(config.bbox[0]): int(1.3 * (config.bbox[0] + config.bbox[2])), 0]
        foreground[int(config.bbox[1]): int(config.bbox[1] + config.bbox[3]),
        int(config.bbox[0]): int(1.3 * (config.bbox[0] + config.bbox[2])),
        1] = foreground_mask_tracked * frame_stab_hsv[int(config.bbox[1]): int(config.bbox[1] + config.bbox[3]),
                                       int(config.bbox[0]): int(1.3 * (config.bbox[0] + config.bbox[2])), 1]
        foreground[int(config.bbox[1]): int(config.bbox[1] + config.bbox[3]),
        int(config.bbox[0]): int(1.3 * (config.bbox[0] + config.bbox[2])), 2] = foreground_tracked
        background = background_hsv

        alpha_full = np.zeros((background_hsv.shape))
        alpha_full[int(config.bbox[1]): int(config.bbox[1] + config.bbox[3]),
        int(config.bbox[0]): int(1.3 * (config.bbox[0] + config.bbox[2])), 0] = alpha
        alpha_full[int(config.bbox[1]): int(config.bbox[1] + config.bbox[3]),
        int(config.bbox[0]): int(1.3 * (config.bbox[0] + config.bbox[2])), 1] = alpha
        alpha_full[int(config.bbox[1]): int(config.bbox[1] + config.bbox[3]),
        int(config.bbox[0]): int(1.3 * (config.bbox[0] + config.bbox[2])), 2] = alpha
    ...
    print("Applying inverse transform to write 'unestablized_alpha.avi'..")
    time.sleep(0.3) #organizing console
    n_frames_alpha = len(alpha_list)
    for i in tqdm(range(n_frames_alpha - 1),position=0, leave=True):
