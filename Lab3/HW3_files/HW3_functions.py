...
def compNormHist(I, S):
    ...
    for channel in range(0,channels):
        ...
        #fix edges from right side - necessary only for the case there are not 15 values (shouldn't be possible but just in case)
...
def quantizise_4(img, N):
    ...
    quant = int(255/N)
    for j in range(0,N):
        mask = (img <= (j+1)*quant) & (img >= (j)*quant) #elementwise AND
        img[mask] = j
    return(img)
...
def showParticles(I, S, W, frame_number, ID):
    ...
    plt.savefig('{0}-{1}.png'.format(ID, frame_number))
